			       ...
			       const G4ThreeVector point, std::ostream &logger )
{
  ...
  if (safeDistance <= 0.0) {
	G4ThreeVector zero(0);
    ReportError( nError, point, zero, safeDistance,"T0: DistanceToIn(p) <= 0", logger );
    return;
  }
			      ...
			      const SBTrunPointList *outside, const G4ThreeVector point, std::ostream &logger )
{
  ...
  if (safeDistance <= 0.0) {
	G4ThreeVector zero(0);
    ReportError( nError, point, zero, safeDistance, "TI: DistanceToOut(p) <= 0", logger );
    return;
  }
  ...
  if (point.x() < visExtent.GetXmin() ||
      ...
      point.z() > visExtent.GetZmax() ) {
		  	G4ThreeVector zero(0);
    ReportError( nError, point, zero, safeDistance, "TI: Point is outside G4VisExtent", logger );
  } 
