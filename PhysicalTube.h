...
template<TranslationIdType tid, RotationIdType rid, class TubeType, class T=double>
...
template<int tid, int rid, class TubeType, typename T=double>
inline
double
PlacedUSolidsTube<tid,rid,TubeType,T>::DistanceToIn( Vector3D const &xm, Vector3D const &vm, double cPstep ) const
{
  ...
  T tolODz, tolIDz;
  tolIDz = tubeparams->fDz - UUtils::GetCarHalfTolerance();
  tolODz = tubeparams->fDz + UUtils::GetCarHalfTolerance();

  Vector3D x,v;
  // do coordinate transformation
  matrix->MasterToLocalCombined<tid,rid>(xm,x,vm,v);
  ...
  if ( abspz >= tolIDz )
  {
    ...
    {
      sd = (abspz - tubeparams->fDz) / std::fabs(v.z); // Z intersect distance
      sd = (sd<0.)? 0. : sd;
  ...
  if ( t1 > 0 )          // Check not || to z axis
  {
    	...
    	if (tubeparams->fRMin)     // Try inner cylinder intersection
    	{
