...
void ProcessNavStates( void* gpu_ptr /* a pointer to buffer of navigation states */, int depth, int n )
{
  ...
  if(i==0){
	printf("SIZEOF NAVSTATE ON THE GPU %d\n", sizeof(vecgeom_cuda::NavigationState));
  }

  // get the navigationstate for this thread/lane
  vecgeom_cuda::NavigationState * state = reinterpret_cast<vecgeom_cuda::NavigationState*>( gpu_ptr +
        vecgeom_cuda::NavigationState::SizeOf(depth)*i ); 
...
void LaunchNavigationKernel( void* gpu_ptr, int depth, int n )
{
  vecgeom_cuda::LaunchParameters launch =
      vecgeom_cuda::LaunchParameters(n);
