/// \file AABBsurf.h
/// Header-only version of AABB templated on the precision type

#ifndef VECGEOM_SURF_AABB_H_
#define VECGEOM_SURF_AABB_H_
...
namespace bvh {
/**
 * @brief Simple struct to represent Axis-Aligned Bounding Boxes (AABB).
...
struct AABBsurf {
  ...
  AABBsurf(Vector3D<Real_t> Min, Vector3D<Real_t> Max) : fMin(Min), fMax(Max) {}
   ...
   */
  VECCORE_ATT_HOST_DEVICE
  Real_t Safety(Vector3D<Real_t> point) const { return ((point - Center()).Abs() - 0.5 * Size()).Max(); }
  ...
  Real_t Distance(Vector3D<Real_t> point, Vector3D<Real_t> direction) const
  {
    ...
    return (tmin < tmax && tmax > 0.0) ? tmin : vecgeom::InfinityLength<Real_t>();
  }
  ...
  bool IntersectInvDirApproach(Vector3D<Real_t> point, Vector3D<Real_t> invdir, Real_t step, Real_t &approach) const
  {
    ...
    approach = (hit && tmin > vecgeom::kToleranceDist<Real_t>) ? vecgeom::MakeMinusTolerantRel(tmin) : Real_t(0);
    return hit;
  }
   ...
   */
  VECCORE_ATT_HOST_DEVICE
  static AABBsurf<Real_t> Union(AABBsurf<Real_t> const &A, AABBsurf<Real_t> const &B)
  {
