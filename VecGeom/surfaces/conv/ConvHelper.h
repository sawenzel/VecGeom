...
bool IsRightSided(vecgeom::Vector3D<Real_t> const &v1,  vecgeom::Vector3D<Real_t> const &v2, vecgeom::Vector3D<Real_t> const &v3)
{
...
bool IsConvexConcave(const int iSect, Container &zArray, Container &rArray, const bool convex, const int nsec)
{
  ...
  if (iSect == 0) {
    ...
    if (convex ^ IsRightSided<Real_t>({rArray[i], zArray[i], 0.}, {rArray[j], zArray[j], 0.}, {rArray[k], zArray[k], 0.})) {
      return false;
    }
  }
