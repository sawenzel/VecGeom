...
template <typename Real_t>
using TrianglePoint = Range<Real_t>;

// Quasi-cross-product for when ranges represent 2D vectors.
template <typename Real_t>
inline Real_t crossProd2D(Range<Real_t> vec1, Range<Real_t> vec2)
{
...
struct RingMask {
  ...
  bool Inside(Vector3D<Real_t> const &local) const
  {
    ...
    auto sysdet  = crossProd2D(vecSPhi, vecEPhi);
    auto divider = 1 / sysdet;
    auto d1      = crossProd2D(localAngle, vecEPhi) * divider;
    auto d2      = crossProd2D(vecSPhi, localAngle) * divider;
...
struct TriangleMask {
  //// TriangleMask format:
  //// p1-3               -> 2D coordinates of the triangle vertices
  //// bp2, bp3           -> 2D coordinates of vertices 2 and 3 relative to vertex 1

  // TODO: Check if the points themselves are useful
  TrianglePoint<Real_t> p1, p2, p3;
  TrianglePoint<Real_t> bp2, bp3;
  ...
  bool Inside(Vector3D<Real_t> const &local) const
  {
    ...
    TrianglePoint<Real_t> blocal{local[0] - p1[0], local[1] - p1[1]};

    auto divider = 1 / crossProd2D(bp2, bp3);
    auto d1      = crossProd2D(blocal, bp3) * divider;
    auto d2      = crossProd2D(bp2, blocal) * divider;
