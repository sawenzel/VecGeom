...
//
//    Vector3D<typename Backend::precision_v> point(x, y, z);
//    saf = rtube.Safety(p, false);
//
//    TubeSafetyToIn<0, 1, Backend, TubeTraits::HollowTubeWithSmallerThanPiSector>(tube, *identity, point, safety_v);
//
//    if(!rtube.Contains(p)) {
//
...
//
//    Vector3D<typename Backend::precision_v> point(x, y, z);
//    saf = rtube.Safety(p, true);
//
//    TubeSafetyToOut<Backend, TubeTraits::HollowTubeWithSmallerThanPiSector>(tube, point, safety_v);
//
//    if(rtube.Contains(p)) {
//
//      if(Abs(saf-safety_v) > 0.0001) {
