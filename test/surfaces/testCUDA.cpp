...
NavStateIndex Locate(Precision x, Precision y, Precision z)
{
  ...
  NavStateIndex state;
  vgbrep::protonav::LocatePointIn(GeoManager::Instance().GetWorld(), pos, state, BrepHelper::Instance().GetSurfData(), true);
  return state;
}

static void TestHost(const SurfData &surfData)
{
  ...
  int exit = 0;
  distance = vgbrep::protonav::ComputeStepAndHit(pos, dir, state, out, surfData, exit);
  safety = vgbrep::protonav::ComputeSafety(pos, state, surfData, exit);
