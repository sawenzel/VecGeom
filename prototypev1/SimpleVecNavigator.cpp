...
PhysicalVolume const * SimpleVecNavigator::LocateGlobalPoint(PhysicalVolume const * vol, Vector3D const & globalpoint, bool top=true)
{
	...
	if( candvolume )
	{
		...
		for(iter=dlist->begin(); iter!=dlist->end(); iter++ )
		{
			...
			if( nextvol->Contains( globalpoint, transformedpoint ) )
			{
				...
				candvolume = LocateGlobalPoint( nextvol, transformedpoint, false );
				break;
			}
		}
	}
	...
	// at the very end: do the matrix multiplications and caching of global matrices -- doing this in a loop will be cache friendly
}
