...
//
//
// $Id: USphere.cc,v 1.90 2010-11-23 14:45:56 gcosmo Exp $
// GEANT4 tag $Name: not supported by cvs2svn $
//
// class USphere
//
// Implementation for USphere class
//
// History:
//
// 05.04.12 M.Kelsey:	 GetPointOnSurface() throw flat in cos(theta), sqrt(r)
// 14.09.09 T.Nikitina: fix for phi section in DistanceToOut(p,v,..),as for UTubs,UCons 
...
USphere& USphere::operator = (const USphere& rhs) 
{
	 ...
	 // Copy data
	 //
	 fRminTolerance = rhs.fRminTolerance; kTolerance = rhs.kTolerance;
...
VUSolid::EnumInside USphere::Inside(const UVector3& p) const
{
	...
	if (!fFullPhiSphere && rho2)	// [fDPhi < 2*UUtils::kPi] and [p.x or p.y]
	{
		...
		else if (pPhi > ePhi + halfAngTolerance)	 { pPhi -= 2*UUtils::kPi; }
		
		if ((pPhi < fSPhi - halfAngTolerance)
			|| (pPhi > ePhi + halfAngTolerance))			{ return in = eOutside; }
		
		else if (in == eInside)	// else it's eSurface anyway already
		{
			if ((pPhi < fSPhi + halfAngTolerance)
				|| (pPhi > ePhi - halfAngTolerance))		{ in = eSurface; }		 
		}
	}
...
bool USphere::Normal(const UVector3& p, UVector3 &n) const
{
	...
	if(radius)	{ nR = UVector3(p.x/radius,p.y/radius,p.z/radius); }
		...
		if (p.x || p.y) // Check if on z axis (rho not needed later)
		{
			...
			else if ((pDistS >= 0) && (pDistE >= 0)) // Outside both *full* phi planes
			{
				...
				else
				{
					...
					if ((compS >= 0) && (compE >= 0)) { sphi = UUtils::Infinity(); }
					else																{ sphi = 0; }
				}		
			}
...
UVector3 USphere::GetPointOnSurface() const
{
	...
	if(fSTheta == 0)	 { aThr=0; }
	if(eTheta == UUtils::kPi) { aFou = 0; }
	if(fSTheta == UUtils::kPi/2) { aThr = UUtils::kPi*(fRmax*fRmax-fRmin*fRmin); }
	if(eTheta == UUtils::kPi/2)	{ aFou = UUtils::kPi*(fRmax*fRmax-fRmin*fRmin); }
...
{
	return UVisExtent(-fRmax, fRmax,-fRmax, fRmax,-fRmax, fRmax);
}
*/

/*
void USphere::DescribeYourselfTo (UVGraphicsScene& scene) const
{
	scene.AddSolid (*this);
}
*/

UPolyhedron* USphere::CreatePolyhedron () const
{
	return new UPolyhedronSphere (fRmin, fRmax, fSPhi, fDPhi, fSTheta, fDTheta);
}

/*
UNURBS* USphere::CreateNURBS () const
{
	return new UNURBSbox (fRmax, fRmax, fRmax);			 // Box for now!!!
}
*/
