...
//
//
// $Id: UTrap.cc 66241 2012-12-13 18:34:42Z gunter $
//
// class UTrap
//
// Implementation for UTrap class
//
// History:
//
...
// 19.04.05 V.Grichine: bug fixed in UTrap("name",UVector3[8] vp)
// 12.12.04 V.Grichine: SurfaceNormal with edges/vertices 
// 15.11.04 V.Grichine: bug fixed in UTrap("name",UVector3[8] vp)
...
// 01.11.96 V.Grichine: Costructor for Right Angular Wedge from STEP, UTrd/Para
	...
	: VUSolid(pName)
{
			 ...
			 pDx2 <= 0 || pDy2 <= 0 || pDx3 <= 0 || pDx4 <= 0 )
	{
		...
		message << "Invalid length parameters for Solid: " << GetName() << std::endl
						...
						<< "					Z - " << pDz;
		// UException("UTrap::UTrap()", "GeomSolids0002",
	...
	: VUSolid(pName)
{
	...
	if (!good)
	{
		...
		// UException("UTrap::UTrap()", "GeomSolids0002", FatalException,
																...
																double pAlp2 )
{
	if ( pDz<=0 || pDy1<=0 || pDx1<=0 || pDx2<=0 || pDy2<=0 || pDx3<=0 || pDx4<=0 )
	{
		...
		message << "Invalid Length Parameters for Solid: " << GetName() << std::endl
						...
						<< "					Y - " << pDy1 << ", " << pDy2 << std::endl
						<< "					Z - " << pDz;
		// UException("UTrap::SetAllParameters()", "GeomSolids0002",
		//						FatalException, message);
	}
	fCubicVolume= 0.;
	fSurfaceArea= 0.;
...
bool UTrap::MakePlanes()
{
	...
	if (!good)
	{
		std::ostringstream message;
		message << "Face at ~-Y not planar for Solid: " << GetName();
		// UException("UTrap::MakePlanes()", "GeomSolids0002",
		//						FatalException, message);
	}
...
bool UTrap::Normal( const UVector3& p, UVector3 &aNormal ) const 
{
	...
	{
#ifdef UCSGDEBUG
		// UException("UTrap::SurfaceNormal(p)", "GeomSolids1002",
	...
	// Set normal
		...
		switch(side)
		{
			...
			default:
				...
				// UException("UTrap::DistanceToOut(p,v,..)","GeomSolids1002",
				//						JustWarning, message);
				break;
	}
	return snxt;
...
double UTrap::SafetyFromOutside(const UVector3& p, bool precise) const
{
	...
	if( Inside(p) == eOutside )
	{
		 ...
		 // UException("UTrap::DistanceToOut(p)",
