   ...
   
{
  ...
  {
    for (int  i=0; i<fgkNofVertices; ++i) {fVertices.push_back(vertices[i]);}
  }
  else
  { 
    for (int  i=0; i <4; ++i) {fVertices.push_back(vertices[3-i]);}
    for (int  i=0; i <4; ++i) {fVertices.push_back(vertices[7-i]);}
  }
 ...
 static const double  halfCarTolerance=VUSolid::fgTolerance*0.5;
  ...
  for (int  i = 0; i < 4; i++)
  {
    ...
    if (len2 > VUSolid::fgTolerance)
    {
      ...
      else if (cross<0.)  { return eOutside; }
    }
    else
    {
      count++;
...
VUSolid::EnumInside UGenericTrap::Inside(const UVector3& p) const
{
  ...
  if (std::fabs(p.z) <= fDz+halfCarTolerance) // First check Z range
  {
    ...
    { 
      if(std::fabs(p.z) > fDz-halfCarTolerance)  { innew=eSurface; }
    }
...
bool UGenericTrap::Normal( const UVector3& p, UVector3 &aNormal ) const
{
  ...
  if ( noSurfaces == 0 )
  {
    ...
    if(Inside(p)!=eSurface){std::cout<<"Point is not on Surface, confirmed by Inside()"<<p<<std::endl;}
    sumnorm=apprnorm;
                                    ...
                                    const int  ipl) const 
{
  ...
  {           
    if (std::fabs(b)<VUSolid::fgTolerance)  { return UUtils::Infinity(); }
    q=-c/b;
    ...
    if (q>-halfCarTolerance)
    {
      if(q<halfCarTolerance)
      {
        ...
        else  // Check second root; return UUtils::Infinity()
        {
          ...
          if (q<=halfCarTolerance) { return UUtils::Infinity(); }
        }
      }
      // Check the Intersection
      //
  ...
  if (std::fabs(p.z)>fDz-halfCarTolerance)
  {
    if (v.z)
    {
      ...
      else
      {
        if(dist[4]<halfCarTolerance)
        {
          if(p.z>0)  { n=UVector3(0,0,1); }
          else         { n=UVector3(0,0,-1); }
          if (n.Dot(v)<0) { dist[4]=0.; }
          else            { dist[4]=UUtils::Infinity(); }
        }
        pt=p+dist[4]*v;
        if (Inside(pt)==eOutside)  { dist[4]=UUtils::Infinity(); }
      }
    }
  }   
                              ...
                              const UVector3& v, const int  ipl) const
{
  ...
  if (Inside(p+v*t) != eSurface)  { t=UUtils::Infinity(); }
...

{
  ...
  {
    distmin=(-fDz-p.z)/v.z;
    side=kMZ; aNormalVector.Set(0,0,-1); 
  }
  else
  {
    if (v.z > 0)
    { 
      distmin = (fDz-p.z)/v.z; 
      side=kPZ; aNormalVector.Set(0,0,1); 
    }
    else            { distmin = UUtils::Infinity(); }
  }      
  ...
  for (int  ipl=0; ipl<4; ipl++)
  {
    ...
    if (std::fabs(a) < VUSolid::fgTolerance)
    {           
      if (std::fabs(b) < VUSolid::fgTolerance) { continue; }
      q=-c/b;
  ...
  {
    if (distmin >= UUtils::Infinity())  { distmin=VUSolid::fgTolerance; }
    ...
    UVector2 xy[4];
    for ( int  j=0; j<4; j++)  { xy[j]=fVertices[i+j]; }
    ...
    { 
      ...
      if (v.z>0) {side= kPZ; aNormalVector.Set(0,0,1);}
      else         { side=kMZ; aNormalVector.Set(0,0,-1);}
...
{
  if(fCubicVolume != 0.) {;}
  else   { fCubicVolume = VUSolid::Capacity(); }
  return fCubicVolume;
}
...
{ 
  ...
  if( (std::fabs(dv.x) < VUSolid::fgTolerance )&&
      (std::fabs(dv.y) < VUSolid::fgTolerance ) )  { return false; }
    
  if( (std::fabs((p4-p3).x) < VUSolid::fgTolerance )&&
      (std::fabs((p4-p3).y) < VUSolid::fgTolerance ) )  { return false; }
  ...
  if (std::fabs(det)<VUSolid::fgTolerance)  //Intersection
  {
    ...
    if ( q < VUSolid::fgTolerance )  { return false; }  // parallel lines
    q = ((dv).Cross(v2)).Mag()/q;
   
    if(q < 1.-VUSolid::fgTolerance)  { return true; }
  }
  return false;
}
